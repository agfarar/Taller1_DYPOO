...
public class CalculadoraEstadisticas
{
	...
	//Mi funcion para dar el pais segun el nombre del atleta
	public String buscarAtletasPorPais(String nombre_atleta) 
	{
		
		String mensaje = "No se econtro el nombre del atleta";
		Pais elPais = null;
		for (int i = 0; i < paises.size() && elPais == null; i++)
		{
			for(Map<String, Object> lista_dict : paises.get(i).consultarAtletas()) {
				if(lista_dict.get("nombre").equals(nombre_atleta))return paises.get(i).darNombre();
			}
		}
		return mensaje;		
	}
